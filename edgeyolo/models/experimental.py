...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        k = 'ema' if ckpt.get('ema') else 'models' if ckpt.get('models') else 'model' if ckpt.get('model') else None
        model.append((ckpt[k] if k is not None else ckpt).float().fuse().eval())  # FP32 models
