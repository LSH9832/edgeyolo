...
{
#ifdef _WIN32
    std::vector<pystring> ret = {".", "./bin", "../bin", "C:/Windows/System32/", "C:/Windows/system32/"};
...
/* ------------------------------------------------ */
    ...
    bool setupByModelSuffix(pystring modelPath, AccPlatform plf)
    {
        auto ss = modelPath.split(".");
        pystring suffix = ss[ss.size()-1].lower();
#ifdef _WIN32
        pystring libName = "yoloInfer";
#else
        pystring libName = "libyoloInfer";
#endif
        if (ACC_PLAT_TENSORRT == plf || (ACC_PLAT_AUTO == plf && (suffix == "engine" || suffix == "trt")))
        {
            libName += "TensorRT";
        }
        else if (ACC_PLAT_MNN == plf || (ACC_PLAT_AUTO == plf && suffix == "mnn"))
        {
            libName += "MNN";
        }
        else if (ACC_PLAT_RKNN == plf || (ACC_PLAT_AUTO == plf && suffix == "rknn"))
        {
            libName += "RKNN";
        }
        else if (ACC_PLAT_ASCEND == plf || (ACC_PLAT_AUTO == plf && suffix == "om"))
        {
            libName += "Ascend";
        }
        else if (ACC_PLAT_HBDNN == plf || (ACC_PLAT_AUTO == plf && suffix == "bin"))
        {
            libName += "hbDNN";
        }
        else if (ACC_PLAT_OPENVINO == plf || (ACC_PLAT_AUTO == plf && 
                 (suffix == "onnx" || suffix == "xml")))
        {
            libName += "OpenVINO";
        }
        else 
...
bool Detector::init(std::string configFile, AccPlatform platform)
{
    if (impl_ == nullptr)
    {
        impl_ = new Impl();
    ...
    if (cfg["platform"].IsDefined())
    {
        std::string platformName = pystring(cfg["platform"].as<std::string>()).lower();
        if (platformName == "hbdnn")
        {
            platform = ACC_PLAT_HBDNN;
        }
        else if (platformName == "openvino")
        {
            platform = ACC_PLAT_OPENVINO;
        }   
        else if (platformName == "mnn")
        {
            platform = ACC_PLAT_MNN;
        }
        else if (platformName == "tensorrt")
        {
            platform = ACC_PLAT_TENSORRT;
        }
        else if (platformName == "rknn")
        {
            platform = ACC_PLAT_RKNN;
        }
        else if (platformName == "ascend")
        {
            platform = ACC_PLAT_ASCEND;
        }
        else
        {
            std::cout << "Unsupported acc platform : " << platformName << std::endl;
            return false;
        }
    }
    ...
    if(!impl_->setupByModelSuffix(modelPath, platform))
    {
    ...
    if (impl_->platform_ == "rknn" || impl_->platform_ == "horizon")
    {
...
void Detector::detect(const cv::Mat& image, std::vector<std::vector<float>>& prediction, cv::Size* oriSz)
{
    ...
    {
        // std::cout << "inference no transpose" << std::endl;
        impl_->inference(impl_->yolo_, resizedImage.data, impl_->preds, ratio);
        // std::cout << "inference no transpose end" << std::endl;
    }
